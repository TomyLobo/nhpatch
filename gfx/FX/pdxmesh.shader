...
PixelShader =
{
	...
	MainCode PixelPdxMeshWaterShip
		ConstantBuffers = { Common, ShipConstants, Shadow }
	[[
		float4 main( VS_OUTPUT_PDXMESHSTANDARD In ) : PDX_COLOR
		{
			float3 RimColor = HSVtoRGB( float3( 0.4f, 0.6f, 0.7f ) );
			const float RimAlpha = 0.02f;
			const float vRimStart = 0.5f;
			const float vRimStop = 0.25f;
			...
			if( vDot > 0.f )
			{
				float vTime = ( vUVAnimationTime + HdrRange_Time_ClipHeight.y ) * 0.03f;
				vColor += tex2D( DiffuseMap, In.vUV0 + vUVAnimationDir * (vTime * 0.25f) );
				vColor += tex2D( DiffuseMap, ( In.vUV0 + float2( 0.20f, -0.13f ) * vTime * 0.01f ) );
			}
...
Effect PdxMeshWaterShip
{
	VertexShader = "VertexPdxMeshStandard"
	PixelShader = "PixelPdxMeshWaterShip"
	BlendState = "BlendStateAlphaBlend"
	Defines = {
		"NO_ALPHA_MULTIPLIED_EMISSIVE"
	}
}
Effect PdxMeshWaterShipShadow
{
	VertexShader = "VertexPdxMeshStandardShadow"
	PixelShader = "PixelPdxMeshStandardShadow"
	Defines = { "IS_SHADOW" }
...
Effect PdxMeshShipTranspShadow
{
	VertexShader = "VertexPdxMeshStandardShadow"
	PixelShader = "PixelPdxMeshStandardShadow"
	Defines = { "IS_SHADOW" }
